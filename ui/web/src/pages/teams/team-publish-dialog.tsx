...
interface TeamPublishDialogProps {
  open: boolean;
...
export function TeamPublishDialog({ open, onOpenChange, team, onConfirm }: TeamPublishDialogProps) {
  ...
  useEffect(() => {
    ...
    setLicense("MIT");
    setAuthor("");
    setForce(readTeamMarketplacePublished(team));
  }, [team, open]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!team || !name.trim() || !version.trim()) return;
    setSubmitting(true);
    try {
      await onConfirm({
        name: name.trim(),
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="grid gap-3 py-2">
            <div className="space-y-1.5">
              <Label htmlFor="team-pub-name">{t("publishDialog.name")}</Label>
              <Input
                id="team-pub-name"
                value={name}
            ...
            <div className="space-y-1.5">
              <Label htmlFor="team-pub-desc">{t("publishDialog.longDescription")}</Label>
              <Textarea
                id="team-pub-desc"
                value={description}
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-1.5">
                <Label htmlFor="team-pub-ver">{t("publishDialog.version")}</Label>
                <Input
                  id="team-pub-ver"
                  value={version}
              ...
              <div className="space-y-1.5">
                <Label htmlFor="team-pub-lic">{t("publishDialog.license")}</Label>
                <Input
                  id="team-pub-lic"
                  value={license}
            ...
            <div className="space-y-1.5">
              <Label htmlFor="team-pub-author">{t("publishDialog.author")}</Label>
              <Input
                id="team-pub-author"
                value={author}

...
func (h *SkillsHandler) uploadOneSkillFromZip(
...
) (map[string]any, *uploadSkillError) {
	userID := store.UserIDFromContext(r.Context())
	stripPrefix := root.stripPrefix

	skillContent, err := readZipFile(root.skillMD)
	if err != nil {
		return nil, &uploadSkillError{Kind: skillUploadErrRead, HTTPStatus: http.StatusBadRequest}
	}
	if strings.TrimSpace(skillContent) == "" {
		return nil, &uploadSkillError{Kind: skillUploadErrEmpty, HTTPStatus: http.StatusBadRequest}
	}

	name, description, slug, frontmatter := skills.ParseSkillFrontmatter(skillContent)
	if name == "" {
		return nil, &uploadSkillError{Kind: skillUploadErrNoName, HTTPStatus: http.StatusBadRequest}
	}
	if slug == "" {
		slug = skills.Slugify(name)
	}
	if !skills.SlugRegexp.MatchString(slug) {
		return nil, &uploadSkillError{Kind: skillUploadErrBadSlug, Slug: slug, HTTPStatus: http.StatusBadRequest}
	}

	if batchSeen != nil {
		if _, dup := batchSeen[slug]; dup {
			return nil, &uploadSkillError{Kind: skillUploadErrDupBatch, Detail: slug, HTTPStatus: http.StatusBadRequest}
		}
		batchSeen[slug] = struct{}{}
	}

	if h.skills.IsSystemSkill(slug) {
		return nil, &uploadSkillError{Kind: skillUploadErrSystem, Slug: slug, HTTPStatus: http.StatusConflict}
	}
	...
	if err := os.MkdirAll(destDir, 0755); err != nil {
		return nil, &uploadSkillError{Kind: skillUploadErrMkdir, Detail: err.Error(), HTTPStatus: http.StatusInternalServerError}
	}
	...
	if err != nil {
		return nil, &uploadSkillError{Kind: skillUploadErrCreate, Detail: err.Error(), HTTPStatus: http.StatusInternalServerError}
	}
